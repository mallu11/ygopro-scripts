...
function c80604091.activate1(e,tp,eg,ep,ev,re,r,rp)
	...
	local ph=Duel.GetCurrentPhase()
	if not (ph~=PHASE_DAMAGE and ph~=PHASE_DAMAGE_CAL and c:IsRelateToEffect(e)) then return end
	if e:GetLabel()~=1 then return end
...
function c80604091.activate2(e,tp,eg,ep,ev,re,r,rp)
	local ph=Duel.GetCurrentPhase()
	if not (ph~=PHASE_DAMAGE and ph~=PHASE_DAMAGE_CAL and e:GetHandler():IsRelateToEffect(e)) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SUMMON)
	local g=Duel.SelectMatchingCard(tp,c80604091.filter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,1,nil)
	local tc=g:GetFirst()
	if tc then
